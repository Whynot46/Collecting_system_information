...
class Enter_Office():
    def __init__(self):
        ...
        self.office_btn = CTk.CTkButton(self.window, text='Экспортировать system info', font=CTk.CTkFont(size=20, weight="bold"), command=self.main)
        ...
        for partition in psutil.disk_partitions():
            ...
            if f"'device': {partition.device}" not in collect_info_dict['info']['disk_info']:
                ...
                collect_info_dict['info']['disk_info'][partition.device] = {'file_system': partition.fstype,
                                                                            'size_total': self.correct_size(
                                                                                partition_usage.total),
                                                                            'size_used': self.correct_size(
                                                                                partition_usage.used),
                                                                            'size_free': self.correct_size(
